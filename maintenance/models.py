...
class Maintenance(models.Model):
    ...
    technician = models.ForeignKey(Technician, on_delete=models.PROTECT, verbose_name=_("technician"),
...
class Periodic(models.Model):
    ...
    sector = models.CharField(max_length=2, choices=Sectors, verbose_name=_("sector"),
                              blank=False, default=Sectors.ENGINE)
    periodicity = models.CharField(max_length=2, choices=Periodicity, default=Periodicity.MONTHLY,
                                   verbose_name=_("periodicity"))
    periodicity_day = models.PositiveIntegerField(choices=DAY_CHOICES, verbose_name=_("day of month"),
                                                  blank=True, null=True)
    periodicity_month = models.PositiveIntegerField(choices=[(i, m) for i, m in enumerate(MONTH_CHOICES, 1)],
                                                    verbose_name=_("month"), blank=True, null=True)
    periodicity_week_day = models.PositiveIntegerField(choices=[(i, d) for i, d in enumerate(WEEK_DAY, 1)],
                                                       verbose_name=_("day of week"), blank=True, null=True)
